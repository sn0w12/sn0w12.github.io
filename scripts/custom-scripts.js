...
function generatePopupContent(
    ...
    const titleLink = linkEnabled
        ? `<a href="wiki/${(linkTitle || title).replace(
              " ",
              "_"
    ...
    const customImg = customImage
        ? `<img style="height: ${fontIncrease - 0.25}em; width: auto;" src="${
              customImage.url
          }" alt="${customImage.alt}" onerror="this.style.display='none';">`
        : region && regionMap[region] && regionMap[region]["titleImg"]
        ? `<img style="height: ${fontIncrease - 0.25}em; width: auto;" src="${
              regionMap[region]["titleImg"].url
          }" alt="${
              regionMap[region]["titleImg"].alt
...
function createAndAddMarker(region, coords, icon, title, id, popupContent) {
    ...
    marker.on("contextmenu", function (e) {
        ...
        function customizeContextMenuForMarker() {
            ...
            let openMarkerDivider =
                document.getElementById("openMarkerDivider");
            ...
            if (href != null) {
                ...
                let openInNewTabBtn =
                    document.getElementById("openWikiInNewTab");
                ...
                document.getElementById("openWikiDivider").style.display =
                    "block";
            }
        }
...
function showCountry(selectedCountry) {
    if (countryPolygons[currentMap]) {
        ...
        const polygon =
            countryPolygons[currentMap][selectedOptionId][selectedCountry];
...
function openMap(map, submap) {
    ...
    if (map != currentMap.toLowerCase()) {
        ...
        let targetLabel = Array.from(labels).find(
            (span) =>
                span.textContent.trim().toLowerCase().replace(" ", "") === map
        );
...
function updateCategorySelection(categories = []) {
    const categorySet = new Set(
        categories.map((category) =>
            category.trim().toLowerCase().replace(" ", "")
        )
    );
    ...
    if (!isPolygon) setSelectValueFromCheckedRadioButton();
        ...
        if (countryPolygons[currentMap][selectedOptionId]) {
            for (const region in countryPolygons[currentMap][
                ...
                const polygon =
                    countryPolygons[currentMap][selectedOptionId][region];
...
function polygonMaker(isDropped = false) {
    ...
    function updateMapDisplay(color) {
        ...
        if (pointsArray.length > 1 && pointsArray.length < 4) {
            ...
            displayPolygon(
                convertArrayToTurfPolygon(pointsArray),
                "",
                false,
                color
            );
    ...
    function drawTemporaryLine() {
        if (pointsArray.length > 0) {
            ...
            addPolylineToMap(
                createLineStringFromPoints(tempPointsArray),
                "blue"
            );
        }
...
function displayPolygon(
        ...
        onEachFeature: function (feature, layer) {
            ...
            if (toolTipTitle != null) {
                layer.bindTooltip(toolTipTitle, {
                    ...
                    offset: [5, 0],
                    opacity: 0,
                    className: `custom-context-menu tooltip ${getTooltipId(
                        toolTipTitle
                    )}`,
                });
            }
    ...
    if (url) {
        ...
        geoJsonLayer.on("mouseup", function (e) {
            ...
            if (!isDragging && e.originalEvent.button === 1) {
                // Middle mouse button
    ...
    if (toolTipTitle != null) {
        geoJsonLayer.on("mouseover", function () {
            let tooltipElement = document.querySelector(
                `.leaflet-tooltip.custom-context-menu.tooltip.${getTooltipId(
                    toolTipTitle
                )}`
            );
            if (tooltipElement) {
    ...
    if (log == true) {
        ...
        let combinedCountryAreas = Object.entries(countryAreas)
            ...
            .map(
                ([region, area]) =>
                    `${region}: ${
                        area.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
                        "Km^2"
                    }`
            );
        ...
        Object.values(group).forEach((marker) => {
            ...
            if (marker.options && marker.options.id) {
                ...
                if (
                    markerId &&
                    !prefixArray.some(
                        (prefix) => markerId.substring(0, 2) === prefix
                    )
                ) {
    ...
    layerGroups.forEach((group) => {
        ...
        layers.forEach((layer) => {
            ...
            subLayers.forEach((subLayer) => {
                let newIconName =
                    subLayer.options.icon.options.className.replace(
                        oldSuffix,
                        ""
                    ) + suffix;
...
function processImportedData(jsonData) {
    return new Promise((resolve, reject) => {
        ...
        performActions(
            ...
            mapConfigurations[currentMap].options[selectedOptionId]
                .checkboxIndex,
            mapConfigurations[currentMap].options[selectedOptionId]
                .checkboxState,
            mapConfigurations[currentMap].options[selectedOptionId]
    ...
    if (typeof dataPolygons !== "undefined") {
        ...
        if (
            ...
            for (const entity in options) {
                ...
                const polygonElement = displayPolygon(
                    polygon,
                    country,
                    false,
                    null,
...
function drawFrontLines(
    ...
    if (
        typeof fontLinePoints === "undefined" ||
    ...
    Object.entries(options).forEach(([frontline, details]) => {
        ...
        if (!drawOnlyArrows) {
            const popupContent = generatePopupContent(
                frontline,
                category,
                generateFrontLineDescription(description),
                linkEnabled,
                linkTitle,
                null
            );
            ...
            let frontLineClickLine = L.polyline(points, {
                color,
                opacity: 0,
                weight: 15,
            }).addTo(map);
            frontLineClickLine.bindPopup(
                `<div id="frontline-popup">${popupContent}</div>`
            );
        ...
        if (drawArrows) {
            ...
            const fontSize = Math.round(
                (Math.log(zoomLevel) / Math.log(2)) * 8
            );
            const density = Math.pow(fontSize, 2) * 0.002;
            const color = countryColors[region] || region;

            let arrows = L.featureGroup(
                getArrows(points, color, density, fontSize, map, rotation, 5)
            ).addTo(map);
...
function setUp(dataUrl) {
    ...
    fetch("contextMenu.html")
        ...
        .then(() => {
            if (
                getFromUrl("country") == null &&
                getFromUrl("markerid") == null
            ) {
                centerMap();
            }
        })
        ...
        if (typeof radioButtons != "undefined") {
            radioButtons.forEach((radioButton) => {
                if (
                    ...
                    radioButton.value == "displayOption2"
                ) {
...
function resetMap() {
    defaultMap =
        neededMaps[0].name.charAt(0).toUpperCase() +
        neededMaps[0].name.slice(1);
...
function createFirstMeasurementPoint(latlng) {
    ...
    tempLine = L.polyline([firstPoint, firstPoint], { color: "red" }).addTo(
        map
    );
...
function drawMeasurementLine(latlng) {
    ...
    var distanceInKm = (
        distance / mapConfigurations[currentMap].meterConversion
    ).toFixed(2);
    var popupContent = `
  ...
  `;
    ...
    overarchingLine.on("contextmenu", function (e) {
        ...
        function customizeContextMenuForLine() {
            ...
            let removeDistanceOpenBtn = document.getElementById(
                "measureDistanceOpen"
            );
    ...
    function createRadioButton(
        ...
        if (clickDeselect) {
            ...
            radio.addEventListener("click", function (e) {
                ...
                if (
                    this.getAttribute("data-last-clicked") === "true" &&
                    this.checked
                ) {
                    this.checked = false;
                    this.setAttribute("data-last-clicked", "false");
                    const deselectionEvent = new CustomEvent(
                        "radioDeselected",
